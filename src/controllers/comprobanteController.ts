...
class ComprobanteController {
    ...
    async consultarDetalle(req: Request, res: Response) {
        const { id } = req.params;
        try {
            const registro = await Comprobante.findOne({ where: { idComprobante: Number(id) }, relations: { usuario: true } });
            if (!registro) {
    ...
    async ingresar(req: Request, res: Response) {
        try {
            ...
            const usuarioRegistro = await Usuario.findOneBy({ idUsuario: Number(usuario) });
            if (!usuarioRegistro) {
    ...
    async actualizar(req: Request, res: Response) {
        ...
        try {
            ...
            const registro = await Comprobante.findOneBy({ idComprobante: Number(id) });
            if (!registro) {
                throw new Error('Curso no encontrado');
            }
            await Comprobante.update({ idComprobante: Number(id) }, req.body);
            const registroActualizado = await Comprobante.findOne({ where: { idComprobante: Number(id) }, relations: { usuario: true } });
